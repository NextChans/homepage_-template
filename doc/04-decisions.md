...
- **결정**
  1. `vercel.json` 에 `{"regions": ["icn1"]}` 을 넣는다. `icn1` = Seoul (AWS `ap-northeast-2`).
...
- **남는 위험**
  ...
  - `vercel.json` 의 리전과 실제 Supabase 리전이 어긋나도 **아무 에러가 나지 않는다.**
    조용히 느려질 뿐이다. 리전을 바꿀 때 두 곳을 함께 고쳐야 한다.
