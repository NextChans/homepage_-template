...
export const site = {
   ...
   *
   * ⚠️ **영어로는 문법에 맞지 않는 문구다.** `Work in trust us` 는 문장으로
   *    성립하지 않는다. 사명의 유래(`Work In Trust` + `us`)를 한 줄에 담으려는
   *    의도이고, **사용자가 이 표기를 유지하기로 결정했다**(2026-09-10).
   *    → 임의로 `WORK IN TRUST` 로 줄이지 말 것. 되돌리려면 사용자에게 확인한다.
   *
   * 헤더에는 넣지 않는다 — 헤더 락업은 `name` + `nameKo` 두 줄까지가 한계다.
   */
  motto: 'WORK IN TRUST US',
